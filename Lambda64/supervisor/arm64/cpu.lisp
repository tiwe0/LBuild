...
(sys.int::define-lap-function %el0-common ()
  ...
  ;; An exception from EL1t (the SP0 vector) enters the common handler using
  ;; SP_EL1, so SP_EL0 is still the interrupted thread's original stack.
...
(sys.int::define-lap-function %elx-common ()
  ...
  ;; Flush the pad slot.
  (mezzano.lap.arm64:str :xzr (:sp #x98))
  ;; The vector entry and register prologue have consumed 0xa0 bytes from
  ;; SP_EL1.  Save the pre-exception value so ERET restores the same kernel
  ;; stack pointer instead of accumulating one frame per interrupt.
  (mezzano.lap.arm64:add :x9 :sp #xa0)
  (mezzano.lap.arm64:str :x9 (:sp #x90))
  ;; Read & save ELR_EL1
  (mezzano.lap.arm64:mrs :x9 :elr-el1)
  (mezzano.lap.arm64:str :x9 (:sp #x78))
  ...
  (mezzano.lap.arm64:ldp :x12 :x11 (:post :sp 16))
  (mezzano.lap.arm64:ldp :x6 :x10 (:post :sp 16))
  ;; x9 contains ELR_EL1 here, not an address into the exception frame.
  ;; The final saved register pair is at the current frame stack pointer,
  ;; matching the EL0 return path above.
  (mezzano.lap.arm64:ldp :x5 :x9 (:post :sp 16))
  (mezzano.lap.arm64:ldr :x29 (:sp))
...
(defun cpu-wired-stack-address-p (address)
  "True when ADDRESS lies inside the local CPU's SP_EL1 wired stack.
The wired stack is per-CPU, not per-thread: every exception entry and every
%CALL-ON-WIRED-STACK-WITHOUT-INTERRUPTS taken on this CPU reuses it.  A thread
resume point stored there is therefore overwritten while the thread sleeps.
The BSP receives its wired stack from the cold generator as a (base . size)
cons; secondary CPUs get a real stack object from %ALLOCATE-STACK."
  (let* ((stack (arm64-cpu-wired-stack (local-cpu-info)))
         (base (if (consp stack) (car stack) (stack-base stack)))
         (size (if (consp stack) (cdr stack) (stack-size stack))))
    (and (<= base address)
         (< address (+ base size)))))
...
(defun detect-secondary-cpus ()
  ;; The current ARM64 FDT child walker can enter an unbounded scan on the
...
(defun initialize-pe-bootstrap-data ()
  ...
  (let* ((bootstrap-frame (%allocate-physical-pages 1 :other "PE bootstrap code" nil))
         ...
         (initial-ttl0-frame (%allocate-physical-pages 1 :other "Initial TTL0" nil))
         (initial-ttl0 (ash initial-ttl0-frame 12))
         (initial-ttl1-frame (%allocate-physical-pages 1 :other "Initial TTL1" nil))
         (initial-ttl1 (ash initial-ttl1-frame 12))
         (initial-ttl2-frame (%allocate-physical-pages 1 :other "Initial TTL2" nil))
    ...
    (setf (physical-memref-unsigned-byte-64 initial-ttl0 (address-l4-bits bootstrap-page))
          (%make-pte initial-ttl1-frame nil t nil nil nil nil :normal))
    (setf (physical-memref-unsigned-byte-64 initial-ttl1 (address-l3-bits bootstrap-page))
          (%make-pte initial-ttl2-frame nil t nil nil nil nil :normal))
    (setf (physical-memref-unsigned-byte-64 initial-ttl2 (address-l2-bits bootstrap-page))
          (%make-pte initial-ttl3-frame nil t nil nil nil nil :normal))
