         ...
         (let ((gc-start (get-internal-run-time))
           ...
           (unwind-protect
                (progn
                  ...
                  ;; FIXUP-TLABS has to run before anything else here.
                  ;; GC-CYCLE released the young oldspace, so every thread's
                  ;; TLAB now points into unmapped memory.  The inline
                  ;; allocation fast path only bumps the TLAB pointer and
                  ;; never consults *GC-IN-PROGRESS*, so an allocation in this
                  ;; window does not trip the "Allocating during GC!" guard --
                  ;; it writes into the hole and takes a memory fault.  Boxing
                  ;; the elapsed time as a float did exactly that, faulting at
