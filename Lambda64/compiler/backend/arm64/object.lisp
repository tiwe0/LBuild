  ...
  (let ((disp (gensym "DISP"))
    ...
    `(cond ((constant-value-p ,index 'fixnum)
            (let ((,disp (object-slot-displacement (fetch-constant-value ,index) ',scale)))
              ;; All object accessors can use the signed 9-bit unscaled form.
              ;; Do not infer scaled-immediate alignment here: tagged object
              ;; addresses make the fixed header displacement non-aligned for
              ;; widths greater than one byte (and the ``unscaled'' accessors
              ;; intentionally use byte offsets).
              (cond ((<= -256 ,disp 255)
