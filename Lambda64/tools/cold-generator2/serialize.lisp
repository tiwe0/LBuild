...
(defmethod allocate-object ((object string) image environment)
  (let* ((area (or (env:object-area environment object)
                   :general))
         ;; Strings use the narrowest character storage that can represent all
         ;; code points.  This matches the runtime's storage-promotion rules.
         (element-size (cond ((every (lambda (character)
                                       (<= (char-code character) #xFF))
                                     object)
                              8)
                             ((every (lambda (character)
                                       (<= (char-code character) #xFFFF))
                                     object)
                              16)
                             (t 32)))
         (element-tag (ecase element-size
                        (8 sys.int::+object-tag-array-unsigned-byte-8+)
    ...
    ;; String data, packed in little-endian character elements.
    (initialize-object-header image data element-tag (length object))
    (dotimes (i (ceiling (* (length object) element-size) 64))
      (let ((value 0))
        (dotimes (j (/ 64 element-size))
          (when (< (+ (* i (/ 64 element-size)) j) (length object))
            (setf (ldb (byte element-size 64) value)
                  (char-code (char object (+ (* i (/ 64 element-size)) j)))))
